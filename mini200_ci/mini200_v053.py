...
def rotate_half(x):
    h=x.shape[-1]//2; return torch.cat((-x[...,h:],x[...,:h]),-1)
...
def apply_rope(q,k,theta):
    ...
    f=torch.outer(p,inv); e=torch.cat((f,f),-1); c=e.cos()[None,None].to(q.dtype); s=e.sin()[None,None].to(q.dtype)
...
class Attention(nn.Module):
    ...
    def forward(self,x):
        B,T,_=x.shape
        q=self.q(x).view(B,T,self.nq,self.hd).transpose(1,2); k=self.k(x).view(B,T,self.nkv,self.hd).transpose(1,2); v=self.v(x).view(B,T,self.nkv,self.hd).transpose(1,2)
        q,k=apply_rope(q,k,self.theta); rep=self.nq//self.nkv
        if rep>1: k=k.repeat_interleave(rep,1); v=v.repeat_interleave(rep,1)
...
class Block(nn.Module):
    ...
    def forward(self,x): x=x+self.a(self.n1(x)); return x+self.m(self.n2(x))
...
class GQACrossAttention(nn.Module):
    ...
    def forward(self,qx,kvx):
        ...
        q=self.q(qx).view(B,S,self.nq,self.hd).transpose(1,2); k=self.k(kvx).view(B,T,self.nkv,self.hd).transpose(1,2); v=self.v(kvx).view(B,T,self.nkv,self.hd).transpose(1,2)
...
class GQASelfAttention(nn.Module):
    ...
    def forward(self,x):
        B,S,_=x.shape
        q=self.q(x).view(B,S,self.nq,self.hd).transpose(1,2); k=self.k(x).view(B,S,self.nkv,self.hd).transpose(1,2); v=self.v(x).view(B,S,self.nkv,self.hd).transpose(1,2)
        q=self.qn(q); k=self.kn(k); rep=self.nq//self.nkv
        if rep>1: k=k.repeat_interleave(rep,1); v=v.repeat_interleave(rep,1)
        y=F.scaled_dot_product_attention(q,k,v,is_causal=False)
        return self.o(y.transpose(1,2).reshape(B,S,-1))
...
class WorkspaceBlock(nn.Module):
    ...
    def forward(self,w,anchor):
        w=w+torch.sigmoid(self.g_cross)*self.cross(self.nq(w),self.nkv(anchor)); w=w+torch.sigmoid(self.g_self)*self.selfa(self.ns(w)); w=w+torch.sigmoid(self.g_ff)*self.ff(self.nf(w)); return w

class PersistentWorkspaceSidecar(nn.Module):
    def __init__(self,cfg):
        super().__init__(); d=cfg.hidden_size; s=cfg.workspace_slots; self.cfg=cfg
        self.anchor_norm=RMSNorm(d,cfg.rms_norm_eps); self.seed_proj=nn.Linear(d,d,bias=False); self.slots=nn.Parameter(torch.randn(1,s,d)*0.02)
        self.blocks=nn.ModuleList([WorkspaceBlock(cfg) for _ in range(cfg.workspace_layers)])
        self.read_norm=RMSNorm(d,cfg.rms_norm_eps); self.read_proj=nn.Linear(d,d,bias=False); self.read_gate=nn.Parameter(torch.tensor(cfg.readout_init))
    def forward_depths(self,anchor,R):
        seed=self.seed_proj(self.anchor_norm(anchor[:,-1:])); w=self.slots.expand(anchor.size(0),-1,-1)+self.cfg.workspace_init_scale*seed; outs=[]
        for _ in range(R):
            for b in self.blocks: w=b(w,anchor)
            delta=self.read_proj(self.read_norm(w[:,0])); h=anchor.clone(); h[:,-1]=h[:,-1]+torch.tanh(self.read_gate)*delta; outs.append(h)
        return outs
...
class Mini200V053(nn.Module):
    def __init__(self,cfg=Mini200V053Config()):
        ...
        self.embed_tokens=nn.Embedding(cfg.vocab_size,cfg.hidden_size); self.layers=nn.ModuleList([Block(cfg) for _ in range(cfg.num_hidden_layers)]); self.sidecar=PersistentWorkspaceSidecar(cfg); self.norm=RMSNorm(cfg.hidden_size,cfg.rms_norm_eps); self.lm_head=nn.Linear(cfg.hidden_size,cfg.vocab_size,bias=False)
    ...
    def forward(self,input_ids,recurrences=1,return_all_depths=False):
        logits=[self.lm_head(h) for h in self.forward_hidden_depths(input_ids,recurrences)]; return logits if return_all_depths else logits[-1]

def architecture_report(cfg=None):
    cfg=cfg or Mini200V053Config()
    with torch.device('meta'): m=Mini200V053(cfg)
    total=sum(p.numel() for p in m.parameters()); side=sum(p.numel() for n,p in m.named_parameters() if n.startswith('sidecar.'))
